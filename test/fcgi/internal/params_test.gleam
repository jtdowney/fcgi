...
pub fn rejects_non_integer_content_length_test() {
  let p = [#("REQUEST_METHOD", "POST"), #("CONTENT_LENGTH", "abc")]
  let assert Error(err) = connection.to_http_request(p, "")
  assert err == connection.InvalidContentLength("abc")
}

pub fn rejects_negative_content_length_test() {
  let p = [#("REQUEST_METHOD", "POST"), #("CONTENT_LENGTH", "-1")]
  let assert Error(err) = connection.to_http_request(p, "")
  assert err == connection.InvalidContentLength("-1")
}
